...
export default function SocialButtons() {
  ...
  const getFriendlyError = (error) => {
    ...
    if (errorCode === "auth/popup-closed-by-user") {
      return "Sign-in popup was closed before login was completed.";
    }
    ...
    if (errorCode === "auth/account-exists-with-different-credential") {
      return "An account already exists with the same email but a different sign-in provider.";
    }
  ...
  return (
    <div>
      ...
      {authError && (
        <div
          role="alert"
          className="mt-3 rounded-lg border border-red-400/30 bg-red-500/10 p-3 text-xs leading-5 text-red-200"
        >
          {authError}
        </div>
      )}
